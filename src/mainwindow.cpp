...
void MainWindow::buildUi()
{
    ...
    m_shortSideCombo->addItem(QStringLiteral("Уменьшение по меньшей стороне: выкл."), 0);
    ...
    m_shortSideCombo->setToolTip(QStringLiteral("Если включено, это единственный режим изменения разрешения: уменьшаются только текстуры, у которых меньшая сторона больше выбранного значения. Апскейла нет; лимит большой стороны профиля в этом режиме не меняет размер."));
    ...
    m_excludeSmallCheck = new QCheckBox(QStringLiteral("Не менять размер маленьких (≤256 по меньшей стороне)"), options);
    ...
    m_excludeNamesEdit->setToolTip(QStringLiteral("Подстроки через запятую. Если путь или имя DDS содержит одну из них, разрешение файла не изменяется. Формат, mipmaps и перекодирование при необходимости остаются доступными."));
    exclusions->addWidget(m_excludeSmallCheck);
    exclusions->addWidget(new QLabel(QStringLiteral("Не менять размер по имени:"), options));
